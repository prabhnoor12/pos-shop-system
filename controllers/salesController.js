...
export async function syncSales(req, res) {
    ...
    try {
        ...
        logEvent('SALES_SYNC_ERROR', { error: err.message });
...
export async function processSale(req, res) {
    try {
        ...
        logEvent('SALE_PROCESS_ERROR', { error: err.message });
...
export async function getSales(req, res) {
    try {
        ...
        if (recipientEmail) {
          sendEmail(recipientEmail, 'Sales Queried', msg, `<p>${msg}</p>`).catch(e => logger.error({ event: 'EMAIL_SEND_ERROR', error: e.message }));
        }
    } catch (err) {
        logEvent('SALES_LIST_ERROR', { error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function getSaleById(req, res) {
    ...
    try {
        ...
        if (recipientEmail) {
          sendEmail(recipientEmail, 'Sale Queried', msg, `<p>${msg}</p>`).catch(e => logger.error({ event: 'EMAIL_SEND_ERROR', error: e.message }));
        }
    } catch (err) {
        logEvent('SALE_GET_ERROR', { id, error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
