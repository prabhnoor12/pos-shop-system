...
export async function createCustomer(req, res) {
    ...
    if (error) {
        logEvent('CUSTOMER_CREATE_FAIL', { reason: error.details[0].message });
    ...
    try {
        ...
        if (exists) {
            logEvent('CUSTOMER_CREATE_FAIL', { reason: 'Customer already exists', email: maskEmail(value.email) });
            return res.status(409).json({ message: 'Customer already exists.' });
        }
        const customer = await prisma.customer.create({ data: { ...value, tenantId } });
        logEvent('CUSTOMER_CREATE_SUCCESS', { id: customer.id, email: maskEmail(customer.email) });
        res.status(201).json(sanitizeCustomer(customer));
    } catch (err) {
        logEvent('CUSTOMER_CREATE_ERROR', { error: err.message });
...
export async function getCustomers(req, res) {
    try {
        ...
        logEvent('CUSTOMER_LIST_ERROR', { error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function updateCustomer(req, res) {
    ...
    if (error) {
        logEvent('CUSTOMER_UPDATE_FAIL', { id, reason: error.details[0].message });
        return res.status(400).json({ message: error.details[0].message });
    }
    try {
        const tenantId = req.user?.tenantId;
        ...
        if (value.email) {
            ...
            if (emailExists) {
                logEvent('CUSTOMER_UPDATE_FAIL', { id, reason: 'Email already in use', email: maskEmail(value.email) });
        ...
        if (value.phone) {
            ...
            if (phoneExists) {
                logEvent('CUSTOMER_UPDATE_FAIL', { id, reason: 'Phone already in use', phone: value.phone });
        ...
        logEvent('CUSTOMER_UPDATE_SUCCESS', { id: customer.id });
        res.json(sanitizeCustomer(customer));
    } catch (err) {
        logEvent('CUSTOMER_UPDATE_ERROR', { id, error: err.message });
...
export async function deleteCustomer(req, res) {
    ...
    try {
        ...
        logEvent('CUSTOMER_DELETE_SUCCESS', { id });
        res.status(200).json(sanitizeCustomer(customer));
    } catch (err) {
        logEvent('CUSTOMER_DELETE_ERROR', { id, error: err.message });
        res.status(404).json({ message: 'Customer not found', error: err.message });
    }
}
