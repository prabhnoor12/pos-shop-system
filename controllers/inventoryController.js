...
export async function listRegisters(req, res) {
    try {
        ...
        logger.error({ event: 'REGISTER_LIST_ERROR', error: err.message });
...
export async function createRegister(req, res) {
    try {
        ...
        logger.error({ event: 'REGISTER_CREATE_ERROR', error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function syncSales(req, res) {
    ...
    try {
        ...
        logger.error({ event: 'SALES_SYNC_ERROR', error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function syncInventory(req, res) {
    ...
    try {
        ...
        logger.error({ event: 'INVENTORY_SYNC_ERROR', error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function batchUpdateInventory(req, res) {
    ...
    try {
        ...
        for (const update of updates) {
            ...
            if (!item) {
                logger.warn({ event: 'BATCH_UPDATE_FAIL', id: update.id, reason: 'Not found or not in tenant' });
            ...
            logger.info({ event: 'BATCH_UPDATE_SUCCESS', id: updated.id });
        ...
        logger.error({ event: 'BATCH_UPDATE_ERROR', error: err.message });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
