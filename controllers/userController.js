...
export async function getUserData(req, res) {
    try {
        ...
        logger.info({ event: 'GDPR_USER_DATA_ACCESS', user: userId });
        res.json({ user });
    } catch (err) {
...
export async function eraseUserData(req, res) {
    try {
        ...
        logger.info({ event: 'GDPR_USER_DATA_ERASURE', user: userId });
...
export async function exportUserData(req, res) {
    try {
        ...
        if (!user) return res.status(404).json({ message: 'User not found' });
        logger.info({ event: 'GDPR_USER_DATA_EXPORT', user: userId });
