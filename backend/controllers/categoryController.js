...
export async function createCategory(req, res) {
    ...
    if (error) {
        logEvent('CATEGORY_CREATE_FAIL', { reason: error.details[0].message, tenantId });
    ...
    try {
        ...
        const exists = await prisma.category.findFirst({ where: { name: value.name, tenantId } });
        if (exists) {
            logEvent('CATEGORY_CREATE_FAIL', { reason: 'Category already exists', name: value.name, tenantId });
            return res.status(409).json({ message: 'Category already exists.' });
        }
        const category = await prisma.category.create({ data: { ...value, tenantId } });
        logEvent('CATEGORY_CREATE_SUCCESS', { id: category.id, name: category.name, tenantId });
        res.status(201).json(sanitizeCategory(category));
    } catch (err) {
        logEvent('CATEGORY_CREATE_ERROR', { error: err.message, tenantId });
...
export async function getCategories(req, res) {
    ...
    try {
        ...
        logEvent('CATEGORY_LIST_ERROR', { error: err.message, tenantId });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function updateCategory(req, res) {
    ...
    const tenantId = req.tenantId;
    if (error) {
        logEvent('CATEGORY_UPDATE_FAIL', { id, reason: error.details[0].message, tenantId });
        return res.status(400).json({ message: error.details[0].message });
    }
    try {
        if (!tenantId) {
        ...
        logEvent('CATEGORY_UPDATE_SUCCESS', { id: category.id, tenantId });
        res.json(sanitizeCategory(category));
    } catch (err) {
        logEvent('CATEGORY_UPDATE_ERROR', { id, error: err.message, tenantId });
...
export async function deleteCategory(req, res) {
    ...
    try {
        ...
        logEvent('CATEGORY_DELETE_SUCCESS', { id, tenantId });
        res.status(200).json(sanitizeCategory(category));
    } catch (err) {
        logEvent('CATEGORY_DELETE_ERROR', { id, error: err.message, tenantId });
        res.status(404).json({ message: 'Category not found', error: err.message });
    }
}
