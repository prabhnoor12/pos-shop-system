...
export async function createSupplier(req, res) {
    ...
    if (error) {
        logEvent('SUPPLIER_CREATE_FAIL', { reason: error.details[0].message, tenantId: req.tenantId });
    ...
    try {
        ...
        if (exists) {
            logEvent('SUPPLIER_CREATE_FAIL', { reason: 'Supplier already exists', name: value.name, tenantId });
            return res.status(409).json({ message: 'Supplier already exists.' });
        }
        const supplier = await prisma.supplier.create({ data: { ...value, tenantId } });
        logEvent('SUPPLIER_CREATE_SUCCESS', { id: supplier.id, name: supplier.name, tenantId });
        res.status(201).json(sanitizeSupplier(supplier));
    } catch (err) {
        logEvent('SUPPLIER_CREATE_ERROR', { error: err.message, tenantId });
...
export async function getSuppliers(req, res) {
    try {
        ...
        logEvent('SUPPLIER_LIST', { tenantId });
        res.json({
        ...
        logEvent('SUPPLIER_LIST_ERROR', { error: err.message, tenantId });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function updateSupplier(req, res) {
    ...
    try {
        ...
        logEvent('SUPPLIER_UPDATE_SUCCESS', { id: supplier.id, tenantId });
        res.json(sanitizeSupplier(supplier));
    } catch (err) {
        logEvent('SUPPLIER_UPDATE_ERROR', { id, error: err.message, tenantId });
...
export async function deleteSupplier(req, res) {
    ...
    try {
        ...
        logEvent('SUPPLIER_DELETE_SUCCESS', { id, tenantId });
        res.status(200).json(sanitizeSupplier(supplier));
    } catch (err) {
        logEvent('SUPPLIER_DELETE_ERROR', { id, error: err.message, tenantId });
        res.status(404).json({ message: 'Supplier not found', error: err.message });
    }
}
