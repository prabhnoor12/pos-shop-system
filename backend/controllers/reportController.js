...
export async function generateSalesReport(req, res) {
    try {
        ...
        logEvent('REPORT_SALES_SUCCESS', { count: sales.length, tenantId });
        res.json({ data: sales });
    } catch (err) {
        logEvent('REPORT_SALES_ERROR', { error: err.message, tenantId: req.tenantId });
...
export async function generateInventoryReport(req, res) {
    try {
        ...
        logEvent('REPORT_INVENTORY_SUCCESS', { count: inventory.length, tenantId });
        res.json({ data: inventory });
    } catch (err) {
        logEvent('REPORT_INVENTORY_ERROR', { error: err.message, tenantId: req.tenantId });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
