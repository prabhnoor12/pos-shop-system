...
export async function assignUserToStore(req, res) {
    try {
        ...
        logger.error({ event: 'STORE_ASSIGN_USER_ERROR', error: err.message, tenantId: req.tenantId });
...
export async function removeUserFromStore(req, res) {
    try {
        ...
        logger.info({ event: 'STORE_USER_REMOVED', storeId, userId, tenantId });
        res.status(204).send();
    } catch (err) {
        logger.error({ event: 'STORE_REMOVE_USER_ERROR', error: err.message, tenantId: req.tenantId });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function getStoreUsers(req, res) {
    try {
        ...
        const users = await prisma.storeUser.findMany({
            where: { storeId: Number(id), tenantId },
            include: { user: true }
        });
        logger.info({ event: 'STORE_GET_USERS', storeId: id, tenantId });
        res.json(users);
    } catch (err) {
        logger.error({ event: 'STORE_GET_USERS_ERROR', error: err.message, tenantId: req.tenantId });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
...
export async function getStoreAnalytics(req, res) {
    try {
        ...
        logger.info({ event: 'STORE_ANALYTICS', storeId: id, tenantId });
        res.json({
        ...
        logger.error({ event: 'STORE_ANALYTICS_ERROR', error: err.message, tenantId: req.tenantId });
        res.status(500).json({ message: 'Server error', error: err.message });
    }
}
// storeController.js
...
export async function getStoreById(req, res) {
    try {
        ...
        logger.info({ event: 'STORE_GET', storeId: id, tenantId });
...
export async function updateStore(req, res) {
    try {
        ...
        logger.info({ event: 'STORE_UPDATED', storeId: id, tenantId });
        res.json(store);
    } catch (err) {
...
export async function deleteStore(req, res) {
    try {
        ...
        logger.info({ event: 'STORE_DELETED', storeId: id, tenantId });
        res.status(204).send();
    } catch (err) {
