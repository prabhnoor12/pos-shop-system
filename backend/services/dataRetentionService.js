...
export async function purgeOldUserData(retentionDays = 30) {
    ...
    // Find users marked as deleted and past retention period, grouped by tenant
    ...
    for (const user of usersToPurge) {
        ...
        await prisma.user.delete({ where: { id: user.id, tenantId: user.tenantId } });
        logger.info({ event: 'GDPR_USER_PURGED', user: user.id, tenantId: user.tenantId });
    }
}
...
export async function purgeOldLogs(retentionDays = 90) {
    const cutoff = new Date(Date.now() - retentionDays * 24 * 60 * 60 * 1000);
    // Purge audit logs per tenant (if auditLog has tenantId)
    ...
    for (const t of tenants) {
        const count = await prisma.auditLog.deleteMany({
            where: { createdAt: { lte: cutoff }, tenantId: t.tenantId }
        });
        logger.info({ event: 'GDPR_AUDIT_LOGS_PURGED', before: cutoff, tenantId: t.tenantId, deleted: count.count });
    }
